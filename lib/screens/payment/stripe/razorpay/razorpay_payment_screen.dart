...
class _RazorPaymentScreenState extends State<RazorPaymentScreen> {
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    EasyLoading.show(status: "ERROR: " + response.code.toString() + " - " + response.message,
    );
  }
    ...
    final orderProvider = Provider.of<OrderProvider>(context);
      return Scaffold(
        appBar: AppBar(
          ...
          title: const Text('Payment using Razorpay',style: TextStyle(color: Colors.white),),
        ),
        body: Center(
            child: Row(
